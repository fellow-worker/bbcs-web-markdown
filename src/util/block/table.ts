...
const getAlignments = (line : string) => {
    return removeClosure(line).split('|').map(value => getAlignment(value));
}
...
const removeClosure = (line : string) => {
    let cleaned = line.trim().substr(1);
    if(cleaned.endsWith('|')) cleaned = cleaned.substr(0, cleaned.length - 2);
    return cleaned;
}